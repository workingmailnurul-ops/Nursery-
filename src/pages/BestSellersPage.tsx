...
export const BestSellersPage: React.FC = () => {
  const { bestSellers } = useFirestore();
  ...
  }, [bestSellers, selectedCategory, searchTerm]);
      ...
      <section className="bg-gradient-to-r from-[#1E3A20] via-[#2F5233] to-[#172D19] text-white py-12 px-4">
        <div className="max-w-5xl mx-auto text-center space-y-3">
          <span className="inline-flex items-center gap-1.5 bg-[#E8862E] text-white text-xs font-black px-3.5 py-1.5 rounded-full uppercase tracking-wider shadow-xs">
            <Star size={14} className="fill-white" /> Top Customer Favorites
          </span>
          <h1 className="text-3xl sm:text-5xl font-serif font-black tracking-tight text-amber-50">
            All-Time Bestselling Saplings
          </h1>
          <p className="text-stone-200 text-xs sm:text-sm max-w-xl mx-auto leading-relaxed">
            Our highest-rated, top-purchased grafted fruit plants with thousands of verified positive customer reviews across India.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div className="bg-white p-5 rounded-2xl border border-stone-200 shadow-2xs flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full sm:w-72">
            <input
              type="text"
              placeholder="Search bestseller plants..."
              value={searchTerm}
              onChange={(e) => {
        ...
        {paginatedProducts.length > 0 ? (
          ...
          <div className="bg-white p-12 text-center rounded-3xl border border-stone-200 text-stone-500">
            <p className="font-bold text-base">No bestsellers found in this selection.</p>
          </div>
        )}
